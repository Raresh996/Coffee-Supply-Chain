    ...
    Ownable
{
    ...
    modifier checkValue(uint256 _upc) {
        ...
        toPayable(items[_upc].consumerID).transfer(amountToReturn);
    }
    ...
    function kill() public {
        if (msg.sender == origOwner) {
            selfdestruct(toPayable(origOwner));
        }
    }
        ...
        checkValue(_upc)
    {
        ...
        address payable farmerPayableAddress =
            toPayable(items[_upc].originFarmerID);

        farmerPayableAddress.transfer(items[_upc].productPrice);
